...
keys.
Step 8: Define a variable called inputValue with the selection of the input element's
value. To this selection apply the parseInt method to convert the value (string) to a number
so that it can be compared with the random number that gets generated by its value and type.
Step 9: The function will check several conditions. In general, the purpose of the game
...
Step 10: We will check if the inputValue is less than the random number and if the numberOfSub
...
Step 11: We will check if the inputValue is greater than the random number and if the numberOfSub
...
Step 12: If none of these previous conditions are met, then we will display a message to the browser
...
Step 13: As long as the user has more chances to submit a guess, the numberOfSub will increase by one.
Step 14: We will use the preventDefault method on the event parameter to prevent the form to be submitted.
